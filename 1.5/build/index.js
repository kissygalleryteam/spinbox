 ...
 * @module spinbox
 **/
KISSY.add('gallery/spinbox/1.5/index',function(S, Node, Base){
    var $ = Node.all;
    var timer = '';
    /**
     * @name SpinBox
    ...
    S.extend(SpinBox, Base,  /** @lends SpinBox.prototype*/{
        ...
        _eventOnChangeNum: function(){
            ...
            $sign.on('keyup keydown mouseup mousedown', function(e){
                var $this = $(this), $parent = $this.parent(1), $target = $parent.children('.' + getCls.init), inputValue = Number(S.trim($target.val())),
                ...
                if(e.type == 'keyup' || e.type == 'mouseup'){
                    ...
                    /*调用外部方法*/
                    if(onValueChange){
                        onValueChange.call(self, {target: $target, trigger: $this});
                    }
                }
        ...
        _eventOnValide: function(){
            var self = this, $target = self.get('target'), onValueChange = self.get('onValueChange');
            var oldValue;
            ...
            $target.on('blur',function(){
                ...
                if(timer) {clearInterval(timer);}
                /*调用外部方法*/
                if( oldValue !== $this.val() && onValueChange){
                    onValueChange.call(self, {target: $this, trigger: $this});
                }
            })
        },
